...
func load_quiz_scene():
	...
	if players_node:
		...
		var username = UserManager.get_logged_in_username()
		if username == "Green":
			var green_player = players_node.find_child("Player1")
			if green_player:
				GameManager.set_green_player(green_player)
		elif username == "Purple":
			var purple_player = players_node.find_child("Player2")
			if purple_player:
				GameManager.set_purple_player(purple_player)
